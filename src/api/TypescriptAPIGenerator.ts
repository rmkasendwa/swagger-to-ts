...
export const generateTypescriptAPI = async ({
  ...
  const scopedCode = Object.entries(requestTagGroupsByScope).reduce<{
    [scopeName: string]: {
      ...
      permissionsCodeConfiguration: PermissionsCodeConfiguration;
  ...
  Object.entries(scopedCode).forEach(([scopeName, scopedCodeConfiguration]) => {
    ...
    //#region Write Permissions files
    if (generateTsEDControllers) {
      console.log(` -> Writing Permissions files...`);
      const permissionsOutputFilePath = `${scopedOutputRootPath}/permissions`;
      const exportableFileNames = Object.entries(permissionsCodeConfiguration)
        .filter(([, { outputCode }]) => {
          return outputCode.length > 0;
        })
        .map(([tag]) => {
          return tagToEntityLabelMappings[tag].PascalCaseEntities;
        });
      if (exportableFileNames.length > 0) {
        ensureDirSync(permissionsOutputFilePath);
      }
      Object.entries(permissionsCodeConfiguration).forEach(
        ([tag, { outputCode }]) => {
          const { PascalCaseEntities } = tagToEntityLabelMappings[tag];
          const permissionsFileName = `${PascalCaseEntities}.ts`;
          const entityPermissionsOutputFilePath = `${permissionsOutputFilePath}/${permissionsFileName}`;
          if (
            outputCode.length > 0 &&
            !existsSync(entityPermissionsOutputFilePath)
          ) {
            writeFileSync(
              entityPermissionsOutputFilePath,
              prettier.format(
                `${autogeneratedFileWarningComment}\n\n${outputCode}`,
                {
                  filepath: entityPermissionsOutputFilePath,
                  ...prettierConfig,
                }
              )
            );
      ...
      const permissionsIndexOutputFilePath = `${permissionsOutputFilePath}/index.ts`;

      if (
        !existsSync(permissionsIndexOutputFilePath) &&
        exportableFileNames.length > 0
      ) {
        const permissionImports: ModuleImports = {};
        const permissionsExportsCode = exportableFileNames
          .map((PascalCaseEntities) => {
            return `export * from './${PascalCaseEntities}';`;
          })
          .join('\n');
        const allPermissionsExportBodyCode = exportableFileNames
          .map((PascalCaseEntities) => {
            addModuleImport({
              imports: permissionImports,
              importFilePath: `./${PascalCaseEntities}`,
              importName: `all${PascalCaseEntities}Permissions`,
            });
            return `...all${PascalCaseEntities}Permissions,`;
          })
          .join('\n');
        writeFileSync(
          permissionsIndexOutputFilePath,
          prettier.format(
            `
              ${autogeneratedFileWarningComment}\n\n
              ${getImportsCode({ imports: permissionImports }).join('\n')}\n
              ${permissionsExportsCode}\n
              //#region All Permissions
              export const allPermissions = [
                ${allPermissionsExportBodyCode}
              ];
              //#endregion
            `.trimIndent(),
            {
              filepath: permissionsIndexOutputFilePath,
              ...prettierConfig,
            }
          )
        );
