...
export const generatePropertySchemaCode = (
  ...
  if ('$ref' in propertySchema) {
    ...
    if (propertyTypes.includes('any')) {
      propertySchemaCodeConfiguration.decorators.push(`@Any()`);
      propertySchemaCodeConfiguration.propertyType = 'any';
      if (generateTsEDControllers) {
        addModuleImport({
          imports,
          importName: 'Any',
      ...
      propertySchemaCodeConfiguration.zodCodeString = `z.any()`;
      propertySchemaCodeConfiguration.isNullable = false;
    } else {
